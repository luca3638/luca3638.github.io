...
function update() {
    ...
    for(let frog of frogs) {
        ...
        // move the fireball
        frog.xPosition += frogXSpeed;

        // remove frog if it goes off the screen
        if(frog.xPosition < destructionXPosition) {
            frogs = frogs.remove(frog);
        }

        if(gameState == "action") {
            // check if the frog collides with the bird
            if(theseCirclesCollide(
                birdXPosition,
                birdYPosition,
                birdHitboxRadius,
    ...
    // spawn new frogs
    if(gameState == "action" &&
    frogTimeSinceLastSpawn>frogSpawnInterval) {
        frogs.push({
            xPosition: spawnXPosition,
            yPosition: randomBetween(0, canvas.height)
        });
        frogTimeSinceLastSpawn = 0;
    }

    if(gameState == "action") {
        frogTimeSinceLastSpawn += timeSinceLastFrame;
    }
